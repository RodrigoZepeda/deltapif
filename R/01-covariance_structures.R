...
covariance_structure_class <- S7::new_class(
  ...
  validator = function(self){
    ...
    if (any(sapply(self@cov_list, length) != n)){
      cli::cli_abort(
        "Element {which(sapply(self@cov_list, length) != n)} of `cov_list` should be of length {n}."
      )
    }
  }
)
...
covariance_structure2 <- function(pif1, pif2, sep = "_>_", add_parents = FALSE, parent_name = "Global"){
  ...
  together_pif <- pif_ensemble(pif1, pif2, weights = c(0, 0), sigma_weights = 0, label = parent_name)
  ...
  if (npifs > 1){
    for (k in 1:npifs){
      for (j in k:npifs){
        ...
        #Case both of them are the same hence assign sigma_weights
        } else if (j == k){
          cov_str[[k]][[k]] <- flat_pif[[j]]@sigma_weights
          ...
          if (!is.null(flat_pif[[j]]@sigma_weights) && !is.null(flat_pif[[k]]@sigma_weights) &&
              !is.null(flat_pif[[j]]@weights) && !is.null(flat_pif[[k]]@weights) &&
              length(flat_pif[[j]]@sigma_weights) == length(flat_pif[[k]]@sigma_weights) &&
              length(flat_pif[[j]]@weights) == length(flat_pif[[k]]@weights) &&
              all(flat_pif[[j]]@sigma_weights == flat_pif[[k]]@sigma_weights) &&
              all(flat_pif[[j]]@weights == flat_pif[[k]]@weights)){

           cov_str[[k]][[j]] <- flat_pif[[j]]@sigma_weights
...
default_parameter_covariance_structure2 <- function(pif1, pif2, sep = "_>_", add_parents = FALSE, parameter = "p", parent_name = "Global"){
  ...
  together_pif <- pif_ensemble(pif1, pif2, weights = c(0, 0),
                               sigma_weights = 0, label = parent_name)
